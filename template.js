...
exports.template = function(grunt, init, done) {
    ...
    init.process({type: 'jquery'}, [
        ...
        function(err, props) {
            ...
            props.dependencies = {jquery: props.jquery_version || '>=3.2.1'};

            // Get only version number from jquery_version string.
            props.jquery_version_strict = '3.2.1';
            var versionRegex = /(\d+\.)?(\d+\.)?(\d+\.)?(\d)/;
            var versionRegexMatches = versionRegex.exec(props.jquery_version);
            if (versionRegexMatches) {
                props.jquery_version_strict = versionRegexMatches[0];
            }

            // GitHub web URL (build from a GitHub repo URI).
            props.repo_url = '';
            var githubUrlRegex = /^.+(?:@|:\/\/)(github.com)[:\/](.+?)(?:\.git|\/)?$/;
            var githubUrlRegexMatches = githubUrlRegex.exec(props.repository);
            if (githubUrlRegexMatches) {
                props.repo_url = 'https://' + githubUrlRegexMatches[1] + '/' + githubUrlRegexMatches[2];
            }
